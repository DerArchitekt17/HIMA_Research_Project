"""
Fine-tune Ministral-3-8B with QLoRA for the swarm Draft-Critique-Refine pipeline.
Trains a single agent (role + SOAP dimension) per invocation.
Auto-scales across all available GPUs via accelerate (data parallelism).

Usage:
    accelerate launch --num_processes <N> train.py --role <role> --agent <agent>
    Requires --role argument: drafter, critic, or refiner.
    Requires --agent argument: subjective, objective, assessment, or plan.
"""
...
VALID_ROLES = ["drafter", "critic", "refiner"]
VALID_AGENTS = ["subjective", "objective", "assessment", "plan"]

# Processing CLI arguments
parser = argparse.ArgumentParser()
parser.add_argument("--role", type=str, required=True, choices=VALID_ROLES,
                    help="Agent role: drafter, critic, or refiner")
parser.add_argument("--agent", type=str, required=True, choices=VALID_AGENTS,
                    help="SOAP dimension: subjective, objective, assessment, or plan")
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "finetuned_adapters", agent, role)
...
dataset = load_dataset(
    "json",
    data_files={
        "train": os.path.join(DATA_DIR, f"training/training_{role}_{agent}.jsonl"),
        "validation": os.path.join(DATA_DIR, f"validation/validation_{role}_{agent}.jsonl"),
    },
)
