...
Trains a single SOAP agent (subjective/objective/assessment/plan) per invocation.
Designed for offline, multi-GPU execution via accelerate + DeepSpeed.

Usage:
    Invoked by run_training.slurm for each SOAP agent sequentially.
...
parser.add_argument("--agent", type=str, required=True, choices=VALID_AGENTS,
                    help="SOAP dimension to train: subjective, objective, assessment, plan")
...
dataset = load_dataset(
    "json",
    data_files={
        "train": os.path.join(DATA_DIR, f"training/training_{agent}.jsonl"),
        "validation": os.path.join(DATA_DIR, f"validation/validation_{agent}.jsonl"),
    },
)
