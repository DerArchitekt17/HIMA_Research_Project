...
Training data for the critic and refiner agents is generated synthetically by cross-pairing drafts within ICD-10 codes (see [data_preperation.ipynb](data_preperation.ipynb)). Each row is paired with another sample sharing the same ICD-10 code via a circular shift; singleton codes — which dominate the validation and test splits due to the stratified split design — are paired across ICD codes as a necessary fallback. These cross-ICD pairs produce less clinically precise synthetic critiques, but still teach the model the critique format and task structure; the model's pre-trained reasoning is expected to refine quality beyond what the synthetic training signal provides. Note that this approximation in the critic and refiner training data is one factor that may limit the DCR pipeline's ability to surpass the simpler single-agent baseline.
...
All six architectures are evaluated on the same 2,006 test cases, enabling **paired** significance tests. Full methodology and per-comparison tables are in [README_STATS.md](README_STATS.md).

**Tests used**: paired t-test (primary) + Wilcoxon signed-rank (non-parametric cross-check) + 95% percentile bootstrap CI (10,000 resamples, seed 0) + Cohen's d effect size.
**Threshold**: α = 0.05, two-tailed, no multiple-comparison correction.

### Architecture Comparison (BERTScore F1, primary metric)

| Comparison | Δ | p (t-test) | p (Wilcoxon) | Cohen's d | Verdict |
|------------|---|------------|--------------|-----------|---------|
| Single-8B vs Multi-8B  | +0.0085 | < 0.001 | < 0.001 | 0.800 | Single significantly better |
| Single-8B vs Swarm-8B  | +0.0092 | < 0.001 | < 0.001 | 0.665 | Single significantly better |
| Multi-8B  vs Swarm-8B  | +0.0007 | 0.024   | 0.161   | 0.050 | **No practical difference** |
| Single-3B vs Multi-3B  | +0.0073 | < 0.001 | < 0.001 | 0.702 | Single significantly better |
| Single-3B vs Swarm-3B  | +0.0086 | < 0.001 | < 0.001 | 0.655 | Single significantly better |
| Multi-3B  vs Swarm-3B  | +0.0013 | < 0.001 | < 0.001 | 0.103 | **Negligible difference**   |

### Scale Comparison (BERTScore F1, 8B vs 3B)

| Comparison | Δ | p (t-test) | p (Wilcoxon) | Cohen's d | Verdict |
|------------|---|------------|--------------|-----------|---------|
| Single-8B vs Single-3B | +0.0015 | < 0.001 | < 0.001 | 0.148 | Small but significant |
| Multi-8B  vs Multi-3B  | +0.0004 | 0.149   | 0.231   | 0.032 | **Not significant**   |
| Swarm-8B  vs Swarm-3B  | +0.0009 | 0.007   | < 0.001 | 0.060 | Negligible            |

**Key takeaways**: (1) The single-agent advantage over multi- and swarm-agent is statistically and practically significant at both scales (medium Cohen's d). (2) Multi-agent and swarm-agent are statistically equivalent — the Wilcoxon test is non-significant for BERTScore (p = 0.161 at 8B), and Cohen's d ≤ 0.103 throughout. (3) Scaling from 3B to 8B yields no significant BERTScore gain in multi- and swarm-agent settings.

### Visualisations

Mean ± 95% bootstrap CI per architecture and metric (generated by `statistical_analysis.ipynb`):

![Bootstrap confidence intervals](statistical_analysis_ci.png)

p-values of all paired t-tests across architectures and metrics:

![p-value heatmap](statistical_analysis_pvalues.png)
